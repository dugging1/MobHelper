...
namespace TaoMobs {
    class BlackJackel : TaoMob {
		...
		public BlackJackel(string name, int num, ImmutableHashSet<IComponent> comps) : this() {
			Name = name;
			Number = num;
			Components = comps;
		}
		public override IMob New(string name, int number, ImmutableHashSet<IComponent> comps) => new BlackJackel(name, number, comps);
